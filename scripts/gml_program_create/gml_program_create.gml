...
{
	var this, l_sources, l_builders, l__g, l_src, l_b, l_main, l__g1, l__g11, l_scr, l__g2, l_tmp, l_w, l_i, l_n, l__g21, l__g3, l_e, l_mcrNames, l_mcrNodes, l_mcrMap, l_i1, l__g5, l_errorNext, l_n1, l_i2;
	
	this = [global.mt_gml_program];
	array_copy(this, 1, global.mq_gml_program, 1, 15);
	l_sources = argument[0];
	array_set(this, 13, 0);
	...
	{
	    l_src = l_sources[l__g];
	    l__g++;
	    l_b = gml_builder_create(l_src);
	    ...
	    if (l_b[9] == undefined)
	    {
	        ...
	        while (l__g1 < array_length_1d(l__g11))
	        {
	            ...
	            if (ds_map_exists(this[3], l_scr[1]))
	            {
	                if (l_scr[1] == l_main)
	                {
	                    l__g2 = gml_std_haxe_boot_wget(ds_map_find_value(this[3], l_main), 4);
	                    
	                    if (l__g2[0] == 89)
	                        l_tmp = array_length_1d(l__g2[2]) == 0;
	                    else
	                        l_tmp = 0;
	        ...
	        l_mcrMap = this[6];
	        l_i1 = 0;
	        l__g5 = array_length_1d(l_mcrNames);
	...
	l_n1 = array_length_1d(this[2]);
	
	for (l_i2 = 0; l_i2 < l_n1; l_i2++)
	    gml_std_haxe_boot_wset(gml_std_haxe_boot_wget(this[2], l_i2), 3, global.g_gml_script_index_offset + l_i2);
