...
{
	var this, l_nodes, l_ops, l_locs, l_proc, l__g, l_pmin, l_pmax, l_n, l_i, l_pcur;
	
	this = argument[0];
	array_set(this, 3, array_get(this, 3) + 1);
	l_nodes = ds_list_create();
	ds_list_add(l_nodes, this[11]);
	l_ops = ds_list_create();
	ds_list_add(l_ops, argument[2]);
	l_locs = ds_list_create();
	...
	l_pmin = 7;
	l_pmax = 0;
	l_n = ds_list_size(l_ops);
	
	for (l_i = 0; l_i < l_n; l_i++)
	{
	    l_pcur = gml_op_get_priority(ds_list_find_value(l_ops, l_i));
