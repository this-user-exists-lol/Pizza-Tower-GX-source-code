...
{
	var this, l__gthis, l_previous, l_yycNext, l_scope, l_args1, l_locals, l_inst, l_with1, l_pos, l_actions, l_len, l_stack, l_proc, l_act, l_n, l_w1, l_i, l_sig, l_keys, l_v2, l_v1, l_op, l__o7, l_f1, l_v9, l_s, l_v3, l__not, l_z, l_v4, l_o2, l_k, l__op, l_ch, l_f4, l__func, l__o2, l_o, l_f5, l_ch1, l_f6, l__func1, l__o3, l__i3, l__o5, l__o6, l__out, l__set, l__get, l__argc, l__o4, l_scr, l_tmp, l_scr2, l_arr, l__argc3, l_cf_ac, l_cf_rc, l_d, l_qn0, l_qn1, l_qc0, l_qc1, l_p1, l_p2, l_p3, l_p4, l_p5, l_jt, l_p6, l__with, l_p8, l__callback;
	
	this = argument[0];
	l__gthis = this;
	l_previous = global.g_gml_thread_current;
	global.g_gml_thread_current = this;
	l_yycNext = "http://bugs.yoyogames.com/view.php?id=24250";
	l_scope = this[4];
	l_args1 = l_scope[3];
	l_locals = l_scope[4];
	l_inst = l_scope[6];
	    ...
	    while (l_proc && l_pos < l_len)
	    {
	        ...
	        switch (l_act[0])
	        {
	            ...
	            case 73:
	                ...
	                if (gml_value_is_number(l_v2))
	                {
	                    ...
	                    if (l_i >= 0 && l_i < 12)
	                    {
	                        ...
	                        if (l_z)
	                        {
	                            if (object_exists(l_v1 | obj_slope))
	                                gml_thread_error("Couldn't find any instances of " + gml_std_Std_string(l_v1) + " (" + object_get_name(l_v1 | obj_slope) + ")");
	                            else
	            ...
	            case 50:
	                ...
	                if (gml_value_is_number(l_v1))
	                {
	                    l_k = (l_v1 | 0) - global.g_gml_script_index_offset;
	                    
	                    if (l_k >= 0)
	                        l_tmp = l_k < array_length_1d(gml_std_haxe_boot_wget(this[0], 2));
	                    else
	                        l_tmp = 0;
	                    ...
	                    {
	                        l_proc = gml_thread_proc_error(l__gthis, "Script index `" + gml_value_print(l_v1) + "` is out of bounds (" + string(global.g_gml_script_index_offset) + ".." + string(global.g_gml_script_index_offset + array_length_1d(gml_std_haxe_boot_wget(this[0], 2))) + " excl.)", l_act);
	                    }
	                }
	                else
	                {
	            ...
	            case 52:
	                ...
	                {
	                    if (l_i < l_d)
	                        l_s = script_execute(l_cf_ac[l_i], l_w1[l_k + l_i]);
	                    else
	                        l_s = script_execute(l_cf_rc, l_w1[l_k + l_i]);
	            ...
	            case 61:
	                l__with = gml_with_func(tools__gml_stack_gml_stack_impl__pop(l_stack));
