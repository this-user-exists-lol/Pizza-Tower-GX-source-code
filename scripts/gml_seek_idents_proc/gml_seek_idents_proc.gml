...
{
	var l_q, l_st, l__g, l_s, l_d, l_scr, l_flags, l__g1, l_k, l_d1;
	...
	if (l__g[0] == 7)
	{
	    l_s = l__g[2];
	    l_d = l__g[1];
	    l_scr = global.g_gml_program_seek_script;
	    ...
	    {
	        l_flags = ds_map_find_value(global.g_gml_var_flags, l_s);
	        
	        if ((l_flags & 4) != 0)
	            gml_std_haxe_enum_tools_set(l_q, [44, l_d, [8, l_d], l_s]);
	        else
	            gml_std_haxe_enum_tools_set(l_q, [41, l_d, l_s]);
	        
	        if (ds_list_size(l_st) > 0)
	        {
	            l__g1 = ds_list_find_value(l_st, 0);
