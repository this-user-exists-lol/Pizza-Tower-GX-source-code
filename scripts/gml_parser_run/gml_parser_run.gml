...
{
	var l_src, l_temStart, l_out, l_row, l_rowStart, l_pos, l_code, l_len, l_buf, l_str, l_sub_buf, l_checkLine, l_brackets, l_c, l_i, l__g, l_start, l_d, l_s, l_n, l_z, l_op, l_tks, l_s_8, l_text;
	
	l_src = argument[0];
	
	if (argument_count > 1)
	    l_temStart = argument[1];
	else
	    l_temStart = -1;
	
	l_out = ds_list_create();
	
	if (l_temStart >= 0)
	{
	    l_row = global.g_gml_parser_tem_row;
	    l_rowStart = global.g_gml_parser_tem_row_start;
	    l_pos = l_temStart;
	}
	else
	{
	...
	l_code = l_src[2];
	l_len = string_byte_length(l_code);
	l_buf = global.g_gml_parser_src_buf;
	l_str = global.g_gml_parser_str_buf;
	...
	l_sub_buf = global.g_gml_parser_buf_sub_buf;
	l_checkLine = 0;
	l_brackets = 0;
	...
	while (l_pos < l_len)
	{
	    ...
	    switch (l_c)
	    {
	        ...
	        case 10:
	            if (l_checkLine)
	            {
	                ...
	                l__g = ds_list_find_value(l_out, l_i);
	    ...
	    switch (l_c)
	    {
	        ...
	        default:
	            if ((l_c >= 97 && l_c <= 122) || (l_c >= 65 && l_c <= 90) || l_c == 95)
	            {
	                ...
	                switch (l_s)
	                {
	                    ...
	                    default:
	                        ...
	                        {
	                            l_s_8 = gml_std_string_substring(l_s, 8);
	            ...
	            {
	                l_text = "Unexpected character `" + chr(l_c) + "`";
	                ds_list_destroy(l_out);
